  ...
  const renderYearList = (brand: string) => {
    ...
    if (carList) {
      return carList.cars.map((item: any, index: number) => (
        <DropdownMenuRadioItem value={item} key={index}>{item.model} {item.year}</DropdownMenuRadioItem>
      ))
    }
    return null
  }
                ...
                <DropdownMenuRadioGroup value={selectedBrand} onValueChange={setSelectedBrand}>
                    {
                        data.map((item: any, index: number) => (
                            <DropdownMenuRadioItem value={item} key={index}>{item.name}</DropdownMenuRadioItem>
                        ))
                    }
                </DropdownMenuRadioGroup>
            </DropdownMenuContent>
